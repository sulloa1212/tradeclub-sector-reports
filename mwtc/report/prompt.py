...
1. Mike's Morning Take [SYNTHESIS] — THIS OPENS THE PAGE (reader's first read; synthesize it from the FULL packet even though the detail sections come after): 2–3 short paragraphs in the Michael Wade voice — what actually matters today, the mindset/risk message, and the one setup or trap to watch. End with the sign-off.
2. Bottom Line — Morning Playbook [SYNTHESIS] — bias · best/weak sectors · opportunity · risk · 3 levels · 3 takeaways. End: "Trade smart. Manage risk. Let the probabilities work for you."
3. Executive Summary (30-sec) [SYNTHESIS] — bias (Bull/Bear/Neutral), confidence 1–10, primary catalysts, biggest opportunity, biggest risk. LEAD-CATALYST RULE: if earnings.overnight_results contains a mega-cap or focus-ticker result from last night, THAT is the primary catalyst — name it first with its beat/miss and surprise %. A trillion-dollar company's after-close print outranks this morning's smaller reporters (on 2026-08-27 the summary led with BBY/DG beats while NVDA's blowout from the prior evening went unmentioned — never repeat that). Absent an overnight earnings result, lead with the biggest overnight story from the news feeds [institutional.news_headlines + overnight_news]. PAIR THE PRINT WITH ITS REACTION: when an overnight_results row carries premarket_reaction_pct (or its ticker sits in [movers]), report the result AND the stock's actual move together — a beat trading DOWN (usually guidance or margins) is a NEGATIVE catalyst; never call it constructive because the EPS line beat (on 2026-09-03 AVGO beat by 8c but fell ~5% overnight on soft guidance, and the summary called it "constructive" risk-on — never repeat that).
<!--DASHBOARD--> goes here (injected — do not draw).
4. Overnight Markets [macro.futures + macro.global_indices] — S&P/Nasdaq/Dow/Russell futures; Asia; Europe. Explain why from packet news.
5. {_GEO}
6. Macro Dashboard [macro.rates_vol + commodities + fx + crypto] — CANONICAL home for 10Y, DXY, Gold, Silver, WTI, Nat Gas, Bitcoin, VIX (one table, one-line implication each).
7. Economic Calendar (today) [macro.economic_calendar; ACTUALS fallback: macro.economic_news] — Time|Event|Consensus|Actual|read. If the structured calendar lacks actual values (gated or lagging the 8:30 release), READ the printed numbers out of macro.economic_news headlines/summaries and show them as Actual, citing the source. NEVER invent a figure — if neither source has it, mark "awaiting".
8. Federal Reserve Watch [fed] — stance, recent commentary, rate path. For market-implied odds on the next meeting, cross-reference the two sources when present: CME futures-implied probabilities [fed.fedwatch] and Kalshi prediction-market odds [fed.kalshi]. From fed.kalshi read the buckets in fed.kalshi.markets (e.g. "Fed maintains rate" / "Cut 25bps" / "Hike 25bps") for fed.kalshi.meeting_date; show each bucket's yes_pct as the headline percentage — it is ALREADY a percent (e.g. 82 → "82%"), do NOT multiply it. (yes_mid is the underlying 0–1 value used for ordering only — never show it as a percent.) When BOTH sources are present, show them side by side and LEAD with the shared takeaway (e.g. "both point to a hold"). A few points of gap between a futures-implied number and a prediction-market number is normal — different crowds, different mechanics — so only call it a real signal if the two disagree on DIRECTION (hold vs cut), not on a few percentage points. Render a panel ONLY for a source whose key is non-null. If only one source is present, show JUST that one, full width — do NOT draw a placeholder box, an empty panel, or any "feed not connected" note for the missing one (a missing odds feed is invisible, never announced). If neither is present, skip the odds display entirely and rely on the qualitative stance from fed.news.
9. Earnings Calendar [earnings.calendar + earnings.overnight_results] — open with a "Reported Since Last Close" group from earnings.overnight_results (last night's after-close + this morning's prints: beat/miss, surprise %, one-line read each — these are the freshest results and set today's tone), then the groups by display_status: Already Reported (verified; beat/miss + surprise %), Upcoming (verified; Before Open/After Close via `when`), and Unconfirmed (verified=false; name only + the `verify_note`, NO date/figures). If earnings.second_source is null, say earnings are single-source/unconfirmed this run.
10. Market News [institutional.news_headlines + overnight_news] — top overnight headlines + impact, merged from BOTH feeds (dedupe overlapping stories). LEAD with the single biggest market-moving story since the last close (a mega-cap earnings result, a macro shock) — never bury it mid-list. If one feed is empty use the other; only if both are empty print "— feed not connected —". TIMESTAMP DISCIPLINE: overnight_news items carry `when_et` and `fresh` — an item tagged BEFORE the last close belongs to the PRIOR session's news cycle: date it explicitly if you use it, never present it as "this morning"/"overnight", and never use a pre-print story (an analyst upgrade, options-flow color) as the market's reaction to a result that printed AFTER it.
11. Sector Rotation [macro.sector_rotation] — rank 11 sectors; strength, trend, bull/bear, driver (drives the ranked chart).
12. Technical Analysis [macro.indices + technicals] — put <!--INDEX-DIALS--> on its own line right under the section title (the SPX/NDX/RUT/DJX bias dials inject there). Then S&P/Nasdaq/Dow/Russell: trend, support, resistance, RSI, key MAs.
13. Options Market & Whale Activity [institutional.options_intel + institutional.per_ticker + institutional.flow_alerts_market + institutional.dark_pool_recent] — THE whale section; use ONLY packet values. Cover: (a) ELEVATED IV (high end of annual range) and LOW IV lists from options_intel.iv_rank.elevated/.low — ticker + IV percentile; print the iv_rank.universe_note so readers know it's a scanned set, not the whole market (elevated = rich premium / sell-vol candidates; low = cheap premium / buy-vol). (b) UNUSUAL volume & OI from options_intel.unusual_contracts — name the tickers, and for each give the exact strike/expiry/type with its volume, OI and vol/OI ratio (volume > OI = fresh positioning). (c) BUSIEST STRIKES per major name from options_intel.top_strikes — highest-volume and highest-OI strike(s) per ticker. (d) WHALE FLOW: biggest sweeps/blocks from flow_alerts_market, net-premium pressure from options_intel.net_prem_ticks, notable dark-pool prints from dark_pool_recent. (e) GEX/dealer positioning from per_ticker.gex_by_strike; put/call from per_ticker.options_volume. Print "— feed not connected —" per missing sub-item only. NEVER invent an IV rank, strike, volume or OI.
14. Market Breadth [breadth] — %>50/200-DMA, advancers/decliners, new highs/lows (or labeled sector proxy).
15. Stocks to Watch [movers + insider + leaders] — 10–15 names derived from packet (not invented).
16. Pre-Market Movers [movers.gainers/losers] — section heading must read "Pre-Market Movers". OPTIONABLE stocks only (data is pre-filtered). List each on its OWN line as "TICKER — Company  +X%", grouped gainers then losers. Add a brief directional read per name where determinable (e.g. momentum-up, gap-fade risk, bearish); omit if not.
17. Analyst Actions [analyst_actions] — upgrades/downgrades/PT changes.
18. Insider Activity [insider] — notable buys/sells (open_market first).
19. Commodities / Currencies / Crypto [macro.*] — brief reads, reference §6 numbers (don't re-quote).
20. Sentiment [sentiment + VIX + put/call] — Fear & Greed, AAII (if present), VIX, P/C; classify mood.
21. Trading Plan for Today [SYNTHESIS] — bull case, bear case, key levels (SPX/QQQ/IWM), defined-risk ideas, invalidation.
22. Weekly Outlook [macro.economic_calendar — ONLY if is_monday] — else omit.
"""

SECTIONS_POSTMARKET = f"""\
1. Mike's Closing Take [SYNTHESIS] — THIS OPENS THE PAGE (reader's first read; synthesize it from the FULL packet even though the detail sections come after): 2–3 short paragraphs in the Michael Wade voice — what today actually told us, the mindset/risk message into tonight/tomorrow, and the one thing to watch. End with the sign-off.
2. Bottom Line — Evening Playbook [SYNTHESIS] — what today means · what to watch overnight/tomorrow · 3 takeaways. End: "Trade smart. Manage risk. Let the probabilities work for you."
3. Closing Bell Summary (30-sec) [SYNTHESIS] — how the day closed (bias, winners/losers), confidence 1–10, the single biggest story of the day (often after-hours earnings), what tomorrow hinges on.
<!--DASHBOARD--> goes here (injected — dials show the NEXT-session lean — do not draw).
4. How the Day Closed [macro.indices + technicals] — put <!--INDEX-DIALS--> on its own line right under the section title (SPX/NDX/RUT/DJX next-session bias dials inject there). Then S&P/Nasdaq/Dow/Russell closing levels & % change; where they finished vs key levels.
5. After-Hours Earnings Reactions [earnings.calendar verified=true AND display_status=reported] — THE headline: who reported after the close, actual vs estimate EPS/rev, beat/miss + surprise %, and the readthrough. Use ONLY verified rows; list any verified=false names separately as "unconfirmed — verify" with no figures. This usually leads the post-market report.
6. {_GEO}
7. Macro Dashboard (close) [macro.rates_vol + commodities + fx + crypto] — closing 10Y, DXY, Gold, Silver, WTI, Nat Gas, Bitcoin, VIX + one-line implication.
8. What Moved & Why [institutional.news_headlines] — the day's key drivers.
8b. Federal Reserve Watch [fed] — one tight section, and ONLY if fed.fedwatch, fed.kalshi, or a Fed-related item in fed.news is present (otherwise omit it entirely — do not print an empty Fed section on a normal day). If the FOMC announced a decision today (visible in fed.news), LEAD with what they decided and the plain-English readthrough for traders. Then show the market-implied odds for the NEXT meeting, cross-referencing CME futures-implied [fed.fedwatch] and Kalshi prediction-market [fed.kalshi] odds: read the fed.kalshi.markets buckets for fed.kalshi.meeting_date and show each bucket's yes_pct as the headline percentage — it is ALREADY a percent, do NOT multiply (yes_mid is the 0–1 underlying, never shown as a percent). Lead with the shared takeaway; a few points of gap between the two is normal, so only flag a real signal if they split on DIRECTION (hold vs cut). Render a panel ONLY for a source whose key is non-null; if only one is present, show JUST that one, full width — never a placeholder box or "feed not connected" note for the missing one.
9. Sector Scorecard [macro.sector_rotation] — how the 11 sectors finished today; leaders & laggards.
10. Post-Market Movers [movers.gainers/losers] — section heading must read "Post-Market Movers". OPTIONABLE stocks only (pre-filtered). Each on its OWN line as "TICKER — Company  +X%"; group day-session vs after-hours. Add a brief directional read per name where determinable; omit if not.
11. Analyst Actions [analyst_actions] — today's rating changes.
12. Insider Activity [insider] — today's notable Form 4 (open_market first).
...
14. Sentiment (close) [sentiment + VIX] — Fear & Greed close, VIX close, mood.
15. Tomorrow's Setup [macro.economic_calendar + earnings(before open) + technicals] — key events tomorrow, names reporting before open, overnight risk, key levels, next-session lean.
"""
